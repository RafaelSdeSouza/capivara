...
.manga_metadata_file <- function(metadata_file = NULL) {
  ...
  installed <- system.file("extdata", file_name, package = "capivaraKinematics", mustWork = FALSE)
  if (nzchar(installed)) {
    candidates <- c(candidates, installed)
  }
  repo <- Sys.getenv("CAPIVARA_REPO_ROOT", unset = "")
  if (nzchar(repo)) {
    candidates <- c(candidates, file.path(repo, "extensions", "capivaraKinematics", "inst", "extdata", file_name))
  }
  candidates <- c(
    candidates,
    file.path(getwd(), "extensions", "capivaraKinematics", "inst", "extdata", file_name),
