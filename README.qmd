...
data cubes. Version `0.3.0` adds built-in missing-data support in the exact
workflow, sparse-Ward segmentation for large cubes, Sagui-style white-light starlet
...
- `segment_large()` for large cubes where the exact all-pairs Ward distance matrix is too expensive in RAM.
...
- both segmentation backends can optionally use a Sagui-style photometric mask built from the white-light image.
...
Use `segment_large()` when that estimate is too large for available RAM.
...
See [NEWS.md](NEWS.md) for the `0.3.0` release summary.
