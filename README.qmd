...
workflow, an SNN graph backend for large cubes, a medoid-based large-cube engine, Sagui-style white-light starlet
...
- `segment_snn()` for large cubes where the exact all-pairs Ward distance matrix is too expensive in RAM.
...
- `segment_snn()` adds a scalable shared-nearest-neighbor backend for large cubes.
